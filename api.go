...
func Load(fileAbsPath, addrPre string) error {
	...
	keystoreStatic = NewKeystore(fileAbsPath, addrPre)
	err := keystoreStatic.Load()
	if err != nil {
		return err
	}
...
@return ok
*/
func GetPukByAddr(addr crypto.AddressCoin, password string) (puk ed25519.PublicKey, ok bool) {
	return keystoreStatic.GetPukByAddr(addr, password)
}
